<div class="row">
    <div class="col-md-4" style="margin-left: 20%;">
        <br>
        <div id="myTabContent" class="tab-content">
            <div class="tab-pane active in" id="home">
                <form id="FormBlog" name="FormBlog" action="/blog/regist" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="actionType" value="<?php echo $data["actionType"]; ?>" />
                    <input type="hidden" name="blog_id" value="<?php echo $data["blog_id"]; ?>" />

                    <div class="form-group">
                        <label>BLOG NAME</label>
                        <input type="text" id="blog_name" name="blog_name" value="<?php echo $data["blog_name"]; ?>" class="form-control" autocomplete="off">
                    </div>
                    <div class="form-group">
                        <label>SUB TITLE</label>
                        <input type="text" name="sub_title" value="<?php echo $data["sub_title"]; ?>" class="form-control" autocomplete="off">
                    </div>
                    <div class="form-group">
                        <label for="header_image">HEADER IMAGE</label><br>
                        <img id="image" src="<?php echo BASEURL."/assets/uploads/blogs/".$data["header_image"]; ?>" alt="your image" onerror="this.src='<?php BASEPATH . DIRECTORY_SEPARATOR . '..'; ?>/assets/images/no-image.jpg'" height="200" width="150" /><br><br>
                        <input type="hidden" name="header_image" value="<?php echo $data["header_image"]; ?>" >
                        <input type="file" class="form-control-file" id="header_image" name="header_image" onchange="readURL(this);">
                    </div>
                    <div class="form-check form-check-inline">
                        <label class="form-check-label" for="is_public">PUBLIC</label>&nbsp;&nbsp;&nbsp;
                        <input type="hidden" name="is_public" value="0" />
                        <input class="form-check-input" type="checkbox" name="is_public" id="is_public" value="1" <?php echo $data["is_public"] ? "checked" : ""; ?>>
                    </div><br><br><br><br>
                </form>
            </div>
        </div>

        <div class="btn-toolbar list-toolbar">
            <a href="/blog" class="btn btn-danger"><i class="glyphicon glyphicon-circle-arrow-left"></i> Back</a>&nbsp;
            <button class="btn btn-primary" onclick="DoSubmit();"><i class="fa fa-save"></i> Save</button>
        </div>
    </div>
</div>

<script type="text/javascript">
    function readURL(input) {
        if (input.files && input.files[0]) {
            var reader = new FileReader();

            reader.onload = function(e) {
                $('#image')
                    .attr('src', e.target.result)
                    .width(150)
                    .height(200);
            };

            reader.readAsDataURL(input.files[0]);
        }
    }

    jQuery(document).ready(function() {
        $('#FormBlog').validate({
            errorElement: 'span', //default input error message container
            errorClass: 'help-inline', // default input error message class
            focusInvalid: false, // do not focus the last invalid input
            ignore: ".ignore",
            rules: {
                blog_name: {
                    required: true
                },
                sub_title: {
                    required: true
                },
            },
            messages: {
                blog_name: " ",
                sub_title: " ",
            },
            highlight: function(element) { // hightlight error inputs
                // $(element).closest('.help-inline').removeClass('ok'); // display OK icon
                $(element).addClass('is-invalid'); // set error class to the control group
            },
            unhighlight: function(element) { // revert the change dony by hightlight
                $(element).removeClass('is-invalid'); // set error class to the control group
            },
            submitHandler: function(form) {}
        });
    });

    function DoSubmit() {
        if (!$("#FormBlog").valid())
            return false;
        document.getElementById("FormBlog").submit();
    }
</script>